 ...
 class Firebase {
   ...
   __buildCollectionQuery(collectionName,sort, order = 'desc', docLimit = 25) {
    ...
    if (!this.__cursors[collectionName]) {
      ...
      collectionRef.orderBy(sort,order).startAt(this.__cursors[collectionName]).limit(docLimit);       
    }
   ...
   async collection(collectionName,sort, order = 'desc', docLimit = 25) {
     let docsArr = []; 
     let collectionRef = this.__buildCollectionQuery(collectionName,sort,order,docLimit);
    ...
    * @returns {void} void
    */
    listenToCollection(collectionName,sort, order = 'desc', docLimit = 25, callback) {
      let collectionQuery = this.__buildCollectionQuery(collectionName,sort,order,docLimit);
      this.__listeners.collections[collectionName] = collectionQuery.onSnapshot( querySnapshot => {
        if (callback) {
